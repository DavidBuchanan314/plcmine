...
#define R(a,b,c,d,e,f,g,h, i) h += S1(e) + Ch2(e,f,g) + sha256_K[i+j] + (j?blk2(i):blk0(i));\
...
#define R(i) sha_h(i) += S1(sha_e(i)) + Ch2(sha_e(i),sha_f(i),sha_g(i)) + sha256_K[i+j] + (j?blk2(i):blk0(i));\
...
static void sha256_block(uint32_t *state, const uint32_t *data)
{
	uint32_t W[16];
	unsigned j;
	#ifdef _SHA256_UNROLL2
...
// avoid polluting the namespace too much
#undef S0
#undef S1
#undef s0
#undef s1
