...
// Specialized base64url-nopad encoder for raw_sig: r_bytes[30..31] || s_bytes[0..31] = 34 bytes -> 46 chars.
// Eliminates the intermediate raw_sig[34] copy buffer.
// 34 bytes = 11 full groups of 3 + 1 remaining byte -> 44 + 2 = 46 chars.
static void b64_raw_sig(uint8_t *out, uint8_t r0, uint8_t r1, const uint8_t *s)
{
    // Group 0: r0, r1, s[0]
    out[0] = B64[(r0>>2)&0x3f];
    out[1] = B64[((r0<<4)|(r1>>4))&0x3f];
    out[2] = B64[((r1<<2)|(s[0]>>6))&0x3f];
    out[3] = B64[s[0]&0x3f];
    // Groups 1-10: s[1..30] (3 bytes each)
    #pragma unroll
    for (int i=0; i<10; i++) {
        uint8_t a=s[1+i*3], b=s[2+i*3], c=s[3+i*3];
        out[4+i*4] = B64[(a>>2)&0x3f];
        out[5+i*4] = B64[((a<<4)|(b>>4))&0x3f];
        out[6+i*4] = B64[((b<<2)|(c>>6))&0x3f];
        out[7+i*4] = B64[c&0x3f];
    }
    // Remainder: s[31] -> 2 chars
    out[44] = B64[(s[31]>>2)&0x3f];
    out[45] = B64[(s[31]<<4)&0x3f];
}
